...
class Notes(generics.GenericAPIView):
    ...
    def get_notes(self, user=None):
        try:
            return NoteModel.objects.filter(note_owner=user)
        except NoteModel.DoesNotExist:
            return None

    def get(self, request):
        notes = self.get_notes(request.user) #get notes for specific user
        ...
        if search_param:
            notes = notes.filter(title__icontains=search_param, note_owner=request.user)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"status": "success", "data": {"note": serializer.data}},
...
class NoteDetail(generics.GenericAPIView):
    ...
    def get_note(self, pk, user=None):
        try:
            return NoteModel.objects.get(pk=pk, note_owner=user)
        except NoteModel.DoesNotExist:
            return None

    def get(self, request, pk):
        user = request.user
        note = self.get_note(pk=pk, user=user)
        if note == None:
            return Response(
    ...
    def patch(self, request, pk):
        note = self.get_note(pk=pk, user=request.user)
        if note == None:
            return Response(
                {"status": "fail", "message": f"Note with Id: {pk} not found"},
                status=status.HTTP_404_NOT_FOUND,
